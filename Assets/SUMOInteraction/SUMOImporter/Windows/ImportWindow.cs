...
namespace SUMOInteraction
{
    ...
    public class ImportWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            terrain = EditorGUILayout.Toggle("Terrain", terrain);
            trees = EditorGUILayout.Toggle("    Trees", trees);
            grass = EditorGUILayout.Toggle("    Grass", grass);
            streets = EditorGUILayout.Toggle("Streets", streets);
            ...
            if (GUILayout.Button("Start"))
            {
                ...
                if (streets)
                {
                    ...
                    ImportAndGenerate.parseNetworkFileLocation(sumoFilesPath);

                    EditorUtility.DisplayProgressBar("Generation Progress", "Generating Street Network", 0.2f);
                    ImportAndGenerate.drawStreetNetwork(sumoFilesPath);
                }
